# https://www.youtube.com/watch?v=QTaou6alCL0&list=PLNIQLFWpQMRXmns-7UarmPIR6DN7bgEzZ&index=29&ab_channel=HarishNeel%7CAI
...
async def get_logistics_info(item: str) -> str:
    """LLM-powered: Return information about a specific logistic item."""
    prompt = (
...
async def generate_intel_scenario(topic: str) -> str:
    """LLM-powered: Generate a creative intelligence scenario based on the topic."""
    prompt = (
...
async def get_tools():
    ...
    return [
        multiply, get_logistics_info, generate_intel_scenario
    ]
...
async def setup_langgraph_app():
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are Rene, a battalion commander agent. You are leading your unit in an operation to attack Russian forces near Maastricht. Your orders must be clear, concise, and actionable. For example, if asked about logistics, you might say: 'Ensure all supply trucks are refueled and ready by 0600 hours.'"),
        ("user", "{input}")
    ])
